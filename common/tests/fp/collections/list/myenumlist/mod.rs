...
fn integration_test_fp_list_myenumlist_cons_case2() {
    ...
    let my_list = 
        MyEnumList::Cons(Rc::new(head), Box::new(my_list));
    println!("my_list {:?}", my_list);
}
...
fn integration_test_fp_list_myenumlist_tail_case2() {
    ...
    let my_list = 
        MyEnumList::Cons(Rc::new(head), Box::new(my_list));
    let expected_tail:Option<Box<MyEnumList<String>>> = 
        Some(Box::new(MyEnumList::Nil()));
...
fn integration_test_fp_list_myenumlist_tail_case3() {
    ...
    let expected_tail = 
        Box::new(MyEnumList::Cons(Rc::new(String::from("Hello")), Box::new(MyEnumList::Nil())));
...
fn performance_test_fp_list_enumlist_cons_case1() {
    ...
    assert_eq!(MyEnumList::head(my_list), 
        Some(Rc::new(String::from("Hello") + &MAX.to_string())));
    println!("integration test performance enumlist cons case1 (millis): {}", 
        elapased_time.as_millis());
    println!("integration test performance enumlist cons case1 (nanos): {}", 
