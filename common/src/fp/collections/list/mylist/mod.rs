...
mod tests {
    ...
    fn test_fp_mylist_head_case3() {
        let x = String::from("x");
        let rc_x = Rc::new(x);
        let mylist: MyList<String> = MyList::Cons(Rc::clone(&rc_x), Box::new(MyList::Nil()));
    ...
    fn test_fp_mylist_head_case4() {
        let x = String::from("x");
        let y: i32 = 123;
        let ip_addr_info = IpAddrInfo {
            0: 123,
            1: String::from("x"),
        };
        let ip_addr = IpAddr { 0: ip_addr_info };
        let rc_ip_addr = Rc::new(ip_addr);
        let mylist: MyList<IpAddr> = MyList::Cons(Rc::clone(&rc_ip_addr), Box::new(MyList::Nil()));
    ...
    fn test_fp_mylist_tail_case4() {
        ...
        let mylist2: MyList<String> = MyList::Cons(Rc::clone(&rc_y), Box::new(mylist));
