...
mod tests {
    ...
    fn test_fp_mystructlist_case2() {
        let mylist: MyStructList<String> = MyStructList {
            values: Vec::from([String::from("Hello")]),
        };
        let mylist2: MyStructList<String> = MyStructList {
            values: Vec::from([String::from("Hello")]),
        };
        assert_eq!(mylist, mylist2);
    }
    ...
    fn test_fp_mystructlist_case4() {
        let ip_addr_info1 = IpAddrInfo {
            0: 1,
            1: String::from("Hello"),
        };
        let ip_addr1 = IpAddr { 0: ip_addr_info1 };

        let ip_addr_info2 = IpAddrInfo {
            0: 1,
            1: String::from("Hello"),
        };
        let ip_addr2 = IpAddr { 0: ip_addr_info2 };

        let mylist: MyStructList<IpAddr> = MyStructList {
            values: Vec::from([ip_addr1]),
        };
        let mylist2: MyStructList<IpAddr> = MyStructList {
            values: Vec::from([ip_addr2]),
        };
        assert_eq!(mylist, mylist2);
    }
}
