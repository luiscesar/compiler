...
fn test_myhashmap_case1() {
    let mymap: MyHashMap<String,String> = 
        MyHashMap{values:HashMap::new()};
    let mymap2: MyHashMap<String,String> = 
        MyHashMap { values: HashMap::new() };
...
fn test_hashmap_string_ptr_case1() {
    ...
    let scopes_pointer = 
        Pointer::new_mut_pointer(scopes);
    ...
    let v = 
        Pointer::clone((*scopes_pointer).borrow().get(0).unwrap().get(&k).unwrap());
...
fn find(key:&StringPtr,scopes_ptr:&Rc<RefCell<Vec<HashMap<StringPtr,StringPtr>>>>) -> 
    Option<StringPtr> {
...
fn find2<K,V>(key:&Rc<K>,scopes_ptr:&Rc<RefCell<Vec<HashMap<Rc<K>,Rc<V>>>>>) -> 
    Option<Rc<V>> where K: Hash+Eq,V:PartialEq {
        let scopes = 
