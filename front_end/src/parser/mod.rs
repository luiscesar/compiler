...
use crate::{lexer::{Lexer, token::{TokenPtr, Token, C_OPEN_ARRAY, Value, C_CLOSE_ARRAY, C_SEMICOLON, Or, And, Eq, Ne, Lt, C_LT, C_GT, ReservedWord, True, False, If, Else, While, Do, Break}}, error::{CompilerError, SyntaxError, messages::{UNDECLARED_IDENTIFIER, INTEGER_EXPECTED, IDENTIFIER_NAME_EXPECTED, ARRAY_DIMENSION_GREATER_THAN_ZERO, EXPECTED_TOKEN, ARITH_EXPR_REQUIRED_UNDER_ARITH_UNARY_OPERATOR, LOGICAL_EXPR_REQUIRED_UNDER_LOGICAL_UNARY_OPERATOR, EXPRESSION_EXPECTED, BREAK_WITHOUT_LOOP}}, parser::stmt::if_stmt::IfStmt};

use self::{stmt::{env::{scope::{ScopeStackMutPtr, new_scope_stack_mut_ptr, Scope, new_scope, find, pop, push}, Env, EnvMutPtr}, StmtPtr, Stmt, seq_stmt::SeqStmt, ifelse_stmt::IfElseStmt, while_stmt::WhileStmt, do_stmt::DoStmt, break_stmt::BreakStmt, set_elem_stmt::SetElemStmt, set_stmt::SetStmt}, expr::{id::{IdPtr, Id}, types::{Type, BasicType, basic_type, basic_type_from_token, Typed, BOOL}, ExprPtr, logical::op::{OR, AND, LogicalUnaryOperator}, Expr, relational::{EQ, NE, LT, RelationalOperator}, arith::op::{ArithBinaryOperator,ArithUnaryOperator}, constant::Constant, access::Access}};
...
impl Parser {
    ...
    fn stmts(&mut self) -> Result<StmtPtr,CompilerError> {
        if *self.look == Token::Char('}') {
            ...
            let seq_stmt_ptr = 
                SeqStmt::new_ptr(&stmt_ptr1,&stmt_ptr2);
    ...
    fn while_stmt(&mut self) -> Result<StmtPtr,CompilerError> {
        ...
        let while_stmt_ptr = 
            WhileStmt::new_ptr(&condition_ptr,&stmt1_ptr)?;
    ...
    fn do_stmt(&mut self) -> Result<StmtPtr,CompilerError> {
        ...
        let do_stmt_ptr = 
            DoStmt::new_ptr(&condition_ptr,&stmt1_ptr)?;
    ...
    fn if_stmt(&mut self) -> Result<StmtPtr,CompilerError> {
        ...
        if *self.look == Else {
            ...
            let if_else_stmt_ptr = 
                IfElseStmt::new_ptr(&condition,&stmt1_ptr,&stmt2_ptr)?;
            ...
            if let Some(id) = id_ref {
                match id.element_type() {
                    Type::Array(basic_type,dimension) => {
                        ...
                        let access_stmt_ptr = 
                            SetElemStmt::new_ptr(&access_ptr, &expr_ptr)?;
    ...
    fn match_int_token(&mut self) -> Result<i32,CompilerError> {
        ...
        match &* token {
            ...
            _ => Err(SyntaxError::compiler_error(ARRAY_DIMENSION_GREATER_THAN_ZERO)),
        }
    }

    fn match_type_token(&mut self,id_basic_type:BasicType) 
    ...
    fn bool(&mut self) -> Result<ExprPtr,CompilerError> {
        ...
        loop {
            if *self.look == Or {
                ...
                expr_ptr = 
                    Expr::new_logical_binary_expr_ptr(OR,&expr_ptr,&expr_ptr2)?;
            } else {
                break;
            }
        }
    ...
    fn join(&mut self) -> Result<ExprPtr,CompilerError> {
        ...
        loop {
            if *self.look == And {
                ...
                expr_ptr = 
                    Expr::new_logical_binary_expr_ptr(AND,&expr_ptr,&expr_ptr2)?;
            } else {
                break;
            }
        }
    ...
    fn equality(&mut self) -> Result<ExprPtr,CompilerError> {
        ...
        loop {
            if *self.look == Eq {
                ...
                expr_ptr = 
                    Expr::new_relational_expr_ptr(EQ, &expr_ptr, &expr_ptr2)?;
            } else if *self.look == Ne {
                self.move_scan()?;
                let expr_ptr2 = self.rel()?;
                expr_ptr = 
                    Expr::new_relational_expr_ptr(NE, &expr_ptr, &expr_ptr2)?;                
            } else {
                break;
            }
        }
    ...
    fn rel(&mut self) -> Result<ExprPtr,CompilerError> {
        ...
        if (*self.look == Token::Char(C_LT)) || 
            ...
            (*self.look == Token::Word(">=")) {
            let op = 
                RelationalOperator::op_from_token(&self.look)?;
            self.move_scan()?;
            let expr_ptr2 = self.expr()?;
            let rel_expr_ptr = 
                Expr::new_relational_expr_ptr(op, &expr_ptr, &expr_ptr2)?;
            Ok(rel_expr_ptr)
        } else {
            Ok(expr_ptr)
        }
    ...
    fn expr(&mut self) -> Result<ExprPtr,CompilerError> { 
        ...
        loop {
            if (*self.look == Token::Char('+')) || 
                (*self.look == Token::Char('-')) {
                    let op = ArithBinaryOperator::op_from_token(&self.look)?;
                    self.move_scan()?;
                    let expr_ptr2 = self.term()?;
                    expr_ptr = 
                        Expr::new_arith_binary_operation_expr_ptr(op, &expr_ptr, &expr_ptr2)?;
            } else {
                break;
            }
        }
    ...
    fn unary(&mut self) -> Result<ExprPtr,CompilerError> {
        if *self.look == Token::Char('-') {
            ...
            if unary_expr_ptr.element_type() != BOOL {
                ...
                let expr_ptr = 
                    Expr::new_arith_unary_operation_expr_ptr(op,&unary_expr_ptr)?;
                Ok(expr_ptr)
            } else {
                Err(SyntaxError::compiler_error(ARITH_EXPR_REQUIRED_UNDER_ARITH_UNARY_OPERATOR))
            }
            ...
            if unary_expr_ptr.element_type() == BOOL {
                ...
                let expr_ptr = 
                    Expr::new_logical_unary_expr_ptr(op, &unary_expr_ptr)?;
                Ok(expr_ptr)
            } else {
                Err(SyntaxError::compiler_error(LOGICAL_EXPR_REQUIRED_UNDER_LOGICAL_UNARY_OPERATOR))
            }
        } else {
    ...
    fn factor(&mut self) -> Result<ExprPtr,CompilerError> {
        ...
        match &*token {
                ...
                if let Some(id) = id_ref {
                    ...
                    match id.element_type() {
                        Type::Array(basic_type,dimension) => {
                            ...
                            self.match_token(&Token::Char(']'))?;
                            let expr_ptr = 
                                Expr::new_access_expr_ptr(&id_name, basic_type, &index)?;
                            Ok(expr_ptr)
                        },
                        _ => {
                            let expr = Expr::ID(id);
            ...
            _ => {
                Err(SyntaxError::compiler_error(EXPRESSION_EXPECTED))
            }
        }
    }
