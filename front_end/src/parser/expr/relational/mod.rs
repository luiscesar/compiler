...
use crate::{
    ...
    lexer::token::{Eq, Ge, Gt, Le, Lt, Ne, Token},
    parser::expr::ExprPtr,
};
...
impl RelationalOperator {
    pub fn op_from_token(token: &Token) -> Result<RelationalOperator, CompilerError> {
        match token {
            ...
            Token::Word("==") => Ok(EQ),
            Token::Word("!=") => Ok(NE),
            _ => Err(SyntaxError::compiler_error(RELATIONAL_OPERATOR_EXPECTED)),
        }
    }
}
    ...
    pub fn new(
        ...
        if relational_condition {
            ...
            Err(SyntaxError::compiler_error(
                RELATIONAL_OPERATION_VALUES_EXPECTED_SAME_ORDERING,
            ))
        }
    }
...
impl ExprT for RelationalOperation {
    fn reduce(&self) -> ExprPtr {
        ...
        let relational_operation = RelationalOperation {
