use std::{rc::Rc, sync::atomic::{AtomicUsize, Ordering}, fmt::{Display, self}};

use common::{pointer::Pointer, collections::string::StringPtr};

use crate::parser::expr::types::{Type, Typed, TypePtr};

use super::{ExprT, types::BOOL, ExprPtr, Expr};
...
impl Id {
    ...
    pub fn new(name:StringPtr,id_type:Type) -> Id {
        Id{name:name,id_type:id_type}
    }

    pub fn new_ptr(name:StringPtr,id_type:Type) -> IdPtr {
        let id = Id{name:name,id_type:id_type};
