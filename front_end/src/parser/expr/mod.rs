...
use self::{
    ...
    arith::{
        op::{ArithBinaryOperation, ArithBinaryOperator, ArithUnaryOperation, ArithUnaryOperator},
        ArithOperation,
    },
    ...
    logical::{
        op::{
            LogicalBinaryOperation, LogicalBinaryOperator, LogicalUnaryOperation,
            LogicalUnaryOperator,
        },
        LogicalOperation,
    },
    relational::{RelationalOperation, RelationalOperationPtr, RelationalOperator},
    types::{BasicType, Type, Typed},
};
...
impl Expr {
    ...
    pub fn new_arith_unary_operation_expr_ptr(
        ...
        let operation_ptr = ArithUnaryOperation::new_ptr(operator, expr)?;
    ...
    pub fn new_logical_binary_expr_ptr(
        ...
        let operation_ptr = LogicalBinaryOperation::new_ptr(operator, left, right)?;
    ...
    pub fn new_logical_unary_expr_ptr(
        ...
        let operation_ptr = LogicalUnaryOperation::new_ptr(operator, expr)?;
        let expr = Expr::LOGICAL(LogicalOperation::UNARY(operation_ptr));
        let expr_ptr = Pointer::new_pointer(expr);
        Ok(expr_ptr)
    }

    pub fn new_relational_expr_ptr(
        operator: RelationalOperator,
        left: &ExprPtr,
        right: &ExprPtr,
    ) -> Result<ExprPtr, CompilerError> {
        let operation_ptr = RelationalOperation::new_pointer(operator, left, right)?;
        let expr = Expr::RELATIONAL(operation_ptr);
        let expr_ptr = Pointer::new_pointer(expr);
        Ok(expr_ptr)
    }

    pub fn new_access_expr_ptr(
