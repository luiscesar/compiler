...
use crate::{parser::expr::{types::{Type, Typed, FLOAT, TypePtr, BOOL}, id::Id, ExprT, ExprPtr, Expr}, error::{CompilerError, SyntaxError, messages::{ARITH_OPERATOR_EXPECTED, ARITH_OPERATION_NUMBERS_EXPECTED}}, lexer::token::Token};
...
impl ArithBinaryOperator {
    pub fn op_from_token(token:&Token) -> Result<ArithBinaryOperator,CompilerError> {
        match token {
            ...
            _ => {
                Err(SyntaxError::compiler_error(ARITH_OPERATOR_EXPECTED))
            }
        }
    }
}
...
impl ArithBinaryOperation {
    pub fn new(op:ArithBinaryOperator,left:&ExprPtr,right:&ExprPtr) -> Result<ArithBinaryOperation,CompilerError> {
        ...
        if (left_type != BOOL) && (right_type != BOOL) {
            ...
            Ok(ArithBinaryOperation { op: op, left:left_expr, right:right_expr})
        } else {
            Err(SyntaxError::compiler_error(ARITH_OPERATION_NUMBERS_EXPECTED))
        }
    }
    pub fn new_ptr(op:ArithBinaryOperator,left:&ExprPtr,right:&ExprPtr) -> 
...
impl ArithUnaryOperation {
    pub fn new(op:ArithUnaryOperator,expr:&ExprPtr) -> Result<ArithUnaryOperation,CompilerError> {
        let expr_type = expr.as_ref().element_type();
        if expr_type != BOOL {
            let expr_clone = Pointer::clone(expr);
            Ok(ArithUnaryOperation {op:op,expr:expr_clone})
        } else  {
            Err(SyntaxError::compiler_error(ARITH_OPERATION_NUMBERS_EXPECTED))
        }
    }
    pub fn new_ptr(op:ArithUnaryOperator,expr:&ExprPtr) -> Result<ArithUnaryOperationPtr,CompilerError> {
