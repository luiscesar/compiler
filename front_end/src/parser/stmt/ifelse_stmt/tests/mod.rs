...
use crate::{
    error::messages::IF_CONDITION_BOOLEAN_EXPECTED,
    parser::{
        expr::{constant::Constant, relational::RelationalOperator, types::INT, Expr},
        stmt::{env::Env, Stmt, StmtType},
    },
};
...
fn test_parser_stmt_ifelse_new_case1() {
    ...
    let condition_result = Expr::new_relational_expr_ptr(RelationalOperator::LT, &x, &one);
    ...
    let ifelse_stmt_ptr_result = IfElseStmt::new_ptr(&condition, &if_stmt, &else_stmt);
...
fn test_parser_stmt_ifelse_new_case2() {
    ...
    let ifelse_stmt_result = IfElseStmt::new_ptr(&x, &if_stmt, &else_stmt);
