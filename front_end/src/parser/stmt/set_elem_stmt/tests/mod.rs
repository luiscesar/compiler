...
use crate::{parser::{expr::{types::{Type, BasicType, basic_type}, constant::Constant, Expr, access::{Access}}, stmt::{env::Env, StmtType}}, error::messages::VALUES_ASSIGNMENT_EQUAL_TYPES};
...
fn test_parser_stmt_set_elem_error_case1() {
    ...
    let expr_ptr = Expr::new_constant_expr_ptr(constant);
    let error = 
        SetElemStmt::new_ptr(&access_ptr,&expr_ptr).unwrap_err();
