...
use crate::{
    error::{messages::WHILE_CONDITION_BOOLEAN_EXPECTED, CompilerError, SyntaxError},
    parser::expr::{
        types::{Typed, BOOL},
        ExprPtr, ExprT,
    },
...
use super::{emit_jump, emit_jump_stmt, emit_label, env::EnvMutPtr, new_label, StmtPtr, StmtType};
    ...
    pub fn new(condition: &ExprPtr, stmt: &StmtPtr) -> Result<WhileStmt, CompilerError> {
        if condition.element_type() == BOOL {
            ...
            Err(SyntaxError::compiler_error(
                WHILE_CONDITION_BOOLEAN_EXPECTED,
            ))
        }
    }
